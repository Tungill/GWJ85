...
func open_pop_up() -> void:
	...
	tween.tween_property(background, "position", Vector2(0.0, 0.0), 0.3).from_current().set_ease(Tween.EASE_OUT)
...
func close_pop_up() -> void:
	var pre_animation_position_x: float = -background_size.x
	var tween: Tween = create_tween()
	tween.tween_property(background, "position", Vector2(pre_animation_position_x, 0.0), 0.5).from_current().set_ease(Tween.EASE_IN)
